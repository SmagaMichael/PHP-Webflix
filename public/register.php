...
//On récupere 4 champs : email, username, password, cf-password
//on vérifie l'email (valide et unqiue dans la bdd), 
//le pseudo (pas vide et unique)
// et le mot de passe qui doit etre egale à cf-password minimum 8 caractère dans un chiffre
//on ajoute l'utilisateur dans la BDD et on fait quelque chose pour le mot de passe 

//traitement
$email = $username = $password = $cfpassword = null;

//Pour sécuriser les mots de passe, on va utiliser la fonction password_hash
//Cette fonction utiliser soit l'algobcrypt  ou argon2...

// $password ='test';
// $hash = password_hash($password, PASSWORD_DEFAULT);
// echo 'LE hash : '.$hash.' correspont au mot de passe '.$password;
...
if(!empty($_POST)){ // Si on soumet le formulaire
    ...
    //Vérifie que l'utilisateur n'existe pas deja 
...
// Les Regex permettent de valider un "format" de chaine
    ...
    //On vérifie que les 2 mots de passe saisie correspondent
    if($password !== $cfpassword){
        $errors['password_4'] = 'Les mots de passe ne correspondent pas ';
    }
    
    //On peut faire la requete SQL
    if(empty($errors)){
        ...
        $query->bindValue(':email', $email);
        $query->bindValue(':username', $username);
        $query->bindValue(':password', password_hash($password, PASSWORD_DEFAULT)); //On stock le hash dans la BDD
        $query->execute();
    ...
    <div class="row">
        <div class="col-lg-6 offset-lg-3">
            <form method="POST">
                ...
                <input type="text" name="username" id="username" class="form-control" value=""><br>

                <label for="password">Mot de passe</label>
                <input type="password" name="password" id="password" class="form-control" value=""><br>

                <label for="email">Conformer le mot de passe</label>
                <input type="password" name="cf-password" id="cf-password" class="form-control" value=""><br>
